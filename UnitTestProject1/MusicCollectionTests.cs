...
namespace UnitTestProject1
{
    ...
    public class MusicCollectionTests
    {
        ...
        // Тест для метода AddTrack()
        [TestMethod]
        public void AddTrack_AddTrackToCollection()
        {
            // Arrange
            ...
            // Assert
            Assert.AreEqual(1, tracks.Count);
            Assert.AreEqual("Hey Jude", tracks[0].Title);
        }

        // Тест для метода RemoveTrack()
        [TestMethod]
        public void RemoveTrack_RemoveTrack()
        {
            // Arrange
            var listView = new ListView();
            var collection = new MusicCollection(listView);
        ...
        // Тест для метода SearchByArtist()
        [TestMethod]
        public void SearchByArtist_FindTrackByArtistName()
        {
            ...
            // Act
            collection.SearchByArtist("The Beatles");

            // Assert
            Assert.AreEqual(1, listView.Items.Count);
            Assert.AreEqual("The Beatles", listView.Items[0].SubItems[0].Text);
        }

        // Тест для метода SortByYear()
        [TestMethod]
